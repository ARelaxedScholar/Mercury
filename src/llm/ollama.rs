    ...
    S: Clone + Send + Sync + 'static,
{
    ...
    pub(crate) async fn execute(self) -> Result<String, LLMError> {
        ...
        // Implicit validation
        let mut cache = self.client.model_cache.ollama.read().unwrap_or_else(|e| e.into_inner()).clone();
        if cache.is_none() {
            if let Ok(models) = self.client.ollama_list_models().await {
                let names: Vec<String> = models.into_iter().map(|m| m.name).collect();
                *self.client.model_cache.ollama.write().unwrap_or_else(|e| e.into_inner()) = Some(names.clone());
                cache = Some(names);
            }
        }
